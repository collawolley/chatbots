...
#U,Sigma,VT=np.linalg.svd(X)           #np自带的svd最大只能处理1000*1000的，用ulimit -s  102400 依然无法解决段错误
#print Sigma
pca = RandomizedPCA(n_components=900)  #900就已经可以达到99%的方差了，如果超过1000，似乎pybrain就会出现运行警告
pca.fit(X)
X_train_pca = pca.transform(X)


print "start save process...."
joblib.dump(pca, 'filename.pkl')       #存储pca模型
f3 = open('./data/X_Train_pca', 'wb')  #存储pca后的X，（numberOfMessage，n_components）列数为pca后压缩后的列数
pickle.dump(X_train_pca, f3)
f3.close
#f4 = open('./data/Y', 'wb')            #存储Y，（numberOfMessage-1，numberOfReducedVocabulary） Y没法用pca压缩，如果压缩了，新的输入进去， 
#pickle.dump(Y, f4)                     #算出来的是压缩后的输出，无法还原，经过实验证明，只要输入变小，Y不变，神经网络的运算已经很快了
#f4.close

#======================================测试是否映射到X正确===================================
z=np.where(X[0]==1)                    #(array([0, 5]),) type(z)=tuple  z[0]=[0,5]
for i in range(0,len(z[0])):   
  print reducedVocabulary[z[0][i]]
#for i in range(350,400):
#  print reducedVocabulary[i]
print pca.explained_variance_ratio_
print pca.explained_variance_ratio_.shape
print sum(pca.explained_variance_ratio_)   #保证达到99%
